...
function toData(v: ProjectFormValues): Prisma.ProjectUncheckedCreateInput {
  ...
  return {
    ...
    features: v.features,
    challenges: orEmpty(v.challenges),
    learnings: orEmpty(v.learnings),
    videoUrl: orEmpty(v.videoUrl),
    architectureImage: orEmpty(v.architectureImage),
    architectureNote: orEmpty(v.architectureNote),
    feedbackQuote: orEmpty(v.feedbackQuote),
    feedbackAuthor: orEmpty(v.feedbackAuthor),
    feedbackRole: orEmpty(v.feedbackRole),
    metaTitle: orEmpty(v.metaTitle),
    metaDescription: orEmpty(v.metaDescription),
    ogImage: orEmpty(v.ogImage),
    deck: v.deck ? (v.deck as Prisma.InputJsonValue) : Prisma.DbNull,
    attachments:
      v.attachments.length > 0 ? (v.attachments as Prisma.InputJsonValue) : Prisma.DbNull,
    demoAccounts:
      v.demoAccounts.length > 0 ? (v.demoAccounts as Prisma.InputJsonValue) : Prisma.DbNull,
